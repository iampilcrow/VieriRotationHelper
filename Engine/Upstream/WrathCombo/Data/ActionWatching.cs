...
public static class ActionWatching
{
    ...
    static unsafe ActionWatching()
    {
        ...
        {
            SendActionHook = Svc.Hook.HookFromSignature<SendActionDelegate>("48 89 5C 24 ?? 48 89 6C 24 ?? 48 89 74 24 ?? 57 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 84 24 ?? ?? ?? ?? 48 8B E9 41 0F B7 D9", ObserveSentAction);
    ...
    private unsafe static void ReceiveActionEffectDetour(uint casterEntityId, Character* casterPtr, Vector3* targetPos, Header* header, TargetEffects* effects, GameObjectId* targetEntityIds)
    {
        ...
        try
        {
            ...
            if (casterEntityId == Player.Object.EntityId &&
                ((actionType == ActionType.Action && ActionSheet.TryGetValue(actionId, out var actionSheet) && actionSheet.TargetArea) || actionType == ActionType.Item))
            {
    ...
    // Observation only: forward the exact call once, before any managed work.
    // Unlike Wrath's execution hook, this cannot replace actions or their targets.
    private static unsafe void ObserveSentAction(ulong targetObjectId, ActionType actionType, uint actionId,
        ushort sequence, long a5, long a6, long a7, long a8, long a9)
    {
        ...
        try
        {
            ...
            var castTime = ActionManager.GetAdjustedCastTime(actionType, actionId);
            ...
            UpdateActionTask = Svc.Framework.RunOnTick(() =>
                ...
                TimeSpan.FromMilliseconds(Math.Max(castTime - 480, 0)), cancellationToken: source.Token);
            if (castTime > 0)
            {
                TimeLastActionUsed = DateTime.Now;
                WrathOpener.CurrentOpener?.ProgressOpener(actionId);
        ...
        catch (Exception ex) { Svc.Log.Error(ex, "Read-only action observation failed"); }
    }
