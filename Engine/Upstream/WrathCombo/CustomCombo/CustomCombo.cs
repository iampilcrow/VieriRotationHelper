...
internal abstract partial class CustomCombo : CustomComboFunctions
{
    ...
    // Read-only entry point: selection is independent of hotbar consolidation,
    // IPC action requests, and the live plugin's enabled/disabled state.
    internal uint Suggest(uint actionID) => Invoke(actionID);
}
