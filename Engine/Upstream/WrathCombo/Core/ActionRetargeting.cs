...
public class ActionRetargeting : IDisposable
{
    ...
    {
        // Suggestions may carry a target resolver, but must never register a
        // hotbar retarget or queue its deferred cleanup callbacks.
        if (ReadOnlyRuntime.Active) return action;
